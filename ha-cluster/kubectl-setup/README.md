...
Bootstrap the FoxIDs HA-cluster stack with plain `kubectl` + `kustomize`.
...
After the Argo CD controllers start, the `meta-application` will sync every manifest under `ha-cluster/app` (matching the Terraform-driven workflow).
