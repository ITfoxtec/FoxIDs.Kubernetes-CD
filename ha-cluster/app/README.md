# FoxIDs HA-cluster Application Deployment

This folder contains the Argo CD GitOps application manifests that together deploy the full HA-cluster FoxIDs environment.
...
- MongoDB single instance (FoxIDs data persistence)
- OpenSearch single instance (audit / log / search storage)
...
## Default Domains (replace test-ha-cluster.foxids.com)
...
- MongoDB runs as a single-instance deployment tuned for simplicity; plan a migration to a dedicated replica set if you need failover.
- OpenSearch runs as a single-instance deployment by default; scale replicas and shards if the workload grows.
...
- Monitor cluster health (Argo CD app statuses, MongoDB pod readiness, OpenSearch green status).
...
Run:
```
kubectl port-forward svc/mongodb -n foxids 27117:27017
```
