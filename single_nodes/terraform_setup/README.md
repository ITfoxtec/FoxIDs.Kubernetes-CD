# Terraform (Single-Node Bootstrap)

Provision the FoxIDs single-node environment with Terraform. Terraform installs the foundational components (namespaces, secrets, Argo CD) and then hands control to Argo CD for the application manifests under `single_nodes/app`.
...
```
cd C:\my-path\FoxIDs.Kubernetes-CD\single_nodes\terraform_setup
```
...
When the apply completes, Argo CD will begin reconciling the manifests under `single_nodes/app`, deploying MongoDB, OpenSearch, FoxIDs, and related ingress resources as single-instance workloads.
